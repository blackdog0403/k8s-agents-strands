...
> 🌐 **Language**: [English](./03-development.md) · **한국어**

[← 02. 아키텍처](./02-architecture.ko.md) · [04. AgentCore 배포 →](./04-deployment-agentcore.ko.md)
...
만약 두 MCP 서버가 같은 이름의 도구를 노출한다면 (드뭅니다), Strands `MCPClient` 의 prefix 옵션을 사용합니다 — [Strands MCP 통합 문서](./00-references.ko.md#1-strands-agents-sdk).
...
- 운영 환경 배포 절차 → [04. AgentCore 배포](./04-deployment-agentcore.ko.md)
- 가독성 원칙과 검토 기준 → [05. 코드 스타일](./05-code-style.ko.md)
- 보안 모델과 운영 체크리스트 → [06. 보안 & 부하](./06-security-and-load.ko.md)

## 더 깊이 알아보기

- Strands `@tool` 데코레이터의 schema 추출 동작 — [Strands MCP Tools 통합](./00-references.ko.md#1-strands-agents-sdk)
- 다른 도메인 MCP 서버를 어떻게 짤지 — [MCP 공식 사이트](./00-references.ko.md#4-model-context-protocol-mcp)
