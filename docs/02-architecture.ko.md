...
> 🌐 **Language**: [English](./02-architecture.md) · **한국어**

[← 01. 시작하기](./01-getting-started.ko.md) · [03. 개발 가이드 →](./03-development.ko.md)

이 문서는 코드가 **왜 이렇게 구성되어 있는지** 를 설명합니다.
세 줄 요약:

1. [Strands Agents SDK](./00-references.ko.md#1-strands-agents-sdk) 로 LLM 에이전트와 도구를 묶습니다.
2. K8s 데이터는 [EKS MCP Server](./00-references.ko.md#3-amazon-eks-mcp-server) 가 가져옵니다 — 우리는 직접 K8s API 를 호출하지 않습니다.
3. 운영 환경에서는 [Bedrock AgentCore Runtime](./00-references.ko.md#2-amazon-bedrock-와-bedrock-agentcore) 위에서 Pattern 4 (full VPC isolation) 로 실행합니다.
...
`Agent(tools=[mcp_client])` 한 줄로 끝납니다. `MCPClient` 가 connect, list_tools, tool 호출, disconnect 라이프사이클을 자동으로 관리합니다 — [Strands MCP Tools 문서](./00-references.ko.md#1-strands-agents-sdk).
...
`infrastructure/redaction.py` 가 토큰·패스워드 패턴 마스킹 유틸을 제공합니다. 현재 1차 방어선은 specialist 의 system prompt 이고, `redact()` 는 layered defense 를 위한 예비 함수입니다 — 자세한 정책은 [06 §6](./06-security-and-load.ko.md).
...
- **새 specialist** (Network, Resource 등) → `agents/<domain>_diagnostic.py` 추가, Orchestrator 의 `tools` 에 등록 ([03 §1](./03-development.ko.md))
- **자체 특화 도구** (MCP 가 못 하는 것) → `tools/` 에 `@tool` 함수 추가, specialist 의 `tools=[mcp, our_tool]` ([03 §2](./03-development.ko.md))
- **다른 MCP 서버 통합** (예: Prometheus MCP) → 같은 패턴으로 `Agent(tools=[eks_mcp, prom_mcp, ...])` ([03 §3](./03-development.ko.md))
...
- 실제로 새 기능을 추가해보기 → [03. 개발 가이드](./03-development.ko.md)
- 운영 환경에 올리기 → [04. AgentCore 배포](./04-deployment-agentcore.ko.md)
- 보안 모델 한 번에 확인하기 → [06. 보안 & 부하](./06-security-and-load.ko.md)

## 더 깊이 알아보기

- Strands 의 agents-as-tools 패턴 — [Strands MCP Tools 통합](./00-references.ko.md#1-strands-agents-sdk)
- Bedrock AgentCore Runtime 의 microVM 격리 모델 — [AgentCore VPC 설정](./00-references.ko.md#2-amazon-bedrock-와-bedrock-agentcore)
- EKS MCP Server 가 호출하는 K8s API 의 범위 — [Amazon EKS MCP Server 소개](./00-references.ko.md#3-amazon-eks-mcp-server)
